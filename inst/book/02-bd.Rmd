...
Los nombres de columnas ya fueron previamente ordenados, clasificados y estandarizados. En un principio, unificando todas las bases de datos desde el 2010, se contaba con `r nrow(columnas)` columnas diferentes. En algunas de ellas era evidente que se trataba de una misma variable con diferencias en el nombre, pero otras no eran tan evidentes.
...
Una primera fase está definida por la homologación que se realiza en base a fuentes de información externas a la base de datos de reclamos de SERNAC. Algunas de ellas guardan relación directa, pero otras corresponden a información proveniente de otros organismos del Estado.
...
Por cada rut en la base de datos del SII, se elige la información más actual (debido a que una misma empresa puede figurar en varios años/periodos diferentes). Luego se unen ambas bases de datos a través del rut del proveedor, sin dígito verificador y a la base de datos princiapl se le agrega la columna `tramo_ventas`, que abarca un total de 13 niveles (más información sobre esta variable puede ser encontrada en el sitio del [SII](http://www.sii.cl/estadisticas/empresas_tamano_ventas.htm). De ser necesario, se pueden añadir otras variables que sean de interés desde los datos del SII.
...
knitr::include_graphics("images/homologacion_warning.png")
```


## Análisis exploratorio {#analisis-exploratorio}

Las variables de interés, a excepción de las fechas, son de tipo categórica (como quedó manifestado en la sección \@ref(seccion-resumen-variables)). Por dicho motivo, realizar un análisis por cada una de ellas no es de mucha utilidad, sobre todo si se considera que el estado actual de la base de datos, no es el que se utilizará para conducir el estudio. Para que los datos cobren sentido, es necesario agruparlos por alguna unidad de tiempo definida (sección \@ref(seccion-seccion-agrupacion)).

Por dicha razón, a continuación se presentarán algunos resumenes de datos agregados, que son de particular interés para contextualizar los reclamos recibidos por SERNAC.

Reclamos por año totales

Reclamos por año del sector
