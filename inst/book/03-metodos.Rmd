...
> "una observación que se desvía en tal grado de otras observaciones, que despierta sospechas de que fue generada por un mecanismo diferente" [@hawkins_identification_1980].
...
Existen varias metodologías para encontrar valores atípicos por medio de análisis descriptivos y/o visuales [@tukey_exploratory_1977; @chambers_graphical_1983; @raschka_python_2016], pero en casos como este, donde el conjunto de datos es grande y supeditado a varias variables y subdivisiones, hacer un análisis visual para cada situación no es viable. 

También es posible encontrar modelos probabilísticos y estadísticos, junto con modelos lineales, para encontrar outliers. Sin embargo, estos asumen una serie de supuestos (distribución, dependencia/independencia) que las características multi seriales de los datos, es díficil de asumir a priori [@aggarwal_outlier_2017].

Algunas técnicas de clasificación no supervisada se basan en la cercanía/lejanía (como los clusters) para encontrar valores atípicos. Sin embargo, al ser el espacio dimensional variable, dependiendo de las métricas que se quieran calcular, no es posible determinar a priori parámetros apropiados para configurar los algoritmos de búsqueda (normalmente el espacio ha oscilado entre 5 a 12 dimensiones/variables). Por otro lado, bajo algunas técnicas se dificulta la identificación de las variables que determinan que dicho registro es una observación fuera de rango, sin considerar que muchos de estos algoritmos están diseñados para encontrar ciertas "formas", que son imposibles de identificar previamente [@aggarwal_outlier_2017].
...
* `lags` se definen las unidades de tiempo a mirar hacia atrás y en este caso, se está mirando a 1, 2, 3, 4, 5, 6, 12 y 24 meses. 
...
1. En total, el número de variables a monitorear aumentar según el número de lags que define el usuario. 
